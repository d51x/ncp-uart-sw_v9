...
# Perenio EFR32MG1 on PL2303 USB-serial
PER_VID = '067B'
PER_PID = '2303'
PER_BAUD = 115200
PER_XON_XOFF = True
...
def ezspV9Init(ser):
    ...
    # EZSP Configuration Frame: version ID: 0x00
    # Note: Must be sent before any other EZSP commands
    # { FRAME CTR + EZSP [0x00 0x00 0x00 0x09] + CRC + FRAME END }
    ser.write(b'\x00\x42\x21\xA8\x5D\x3C\x81\x7E')
    # Wait for Data Response { protocolVersion, stackType, stackVersion }
    # this must be ACK'd
    resp = ser.read(11)
    # DATA ACK response frame
    ...
    # Check ncp data response:
    #                      FRC  |       EZSP        |     Data     |  -CRC-  | Flag
    # 9.0.0 ncp example: { 0x01 0x42 0xA1 0xA8 0x5D 0x28 0x15 0xc3 0x30 0x86 0x7E }
    if resp[1:5] != b'\x42\xA1\xA8\x5D':
        return ERROR_NCP_WRONG_EZSP_VERSION
    # EZSP v9 init ok
    global isV9 
    isV9 = True
...
def flash(port, file):
    ...
    for p in serial.tools.list_ports.comports():
	    ...
	    # Check if Perenio EFR32MG1 on PL2303 USB-serial
            if vid == PER_VID and pid == PER_PID:
                print('Perenio EFR32MG1 on PL2303 USB-serial')
                BAUD = PER_BAUD
                XON_XOFF = PER_XON_XOFF
                RTS_CTS = PER_RTS_CTS
                break
    ...
    if ezspV9Init(ser)!=NO_ERROR and ezspV8Init(ser)!=NO_ERROR and ezspV7Init(ser)!=NO_ERROR and ezspV6Init(ser)!=NO_ERROR and ezspV5Init(ser)!=NO_ERROR and ezspV4Init(ser)!=NO_ERROR:
...
def scan():
    ...
    for port in serial.tools.list_ports.comports():
        ...
        try:
            ...
            if vid == PER_VID and pid == PER_PID or vid == CEL_VID and pid == CEL_PID or vid == WSTK_VID and pid == WSTK_PID or vid == ETRX_VID and pid == ETRX_PID or vid == SONOFF_VID and pid == SONOFF_PID:
                # Use EM3588 USB stick as default
                BAUD = CEL_BAUD
                XON_XOFF = CEL_XON_XOFF
                RTS_CTS = CEL_RTS_CTS
		# Check if Perenio board
                if vid == PER_VID and pid == PER_PID:
                    BAUD = PER_BAUD
                    XON_XOFF = PER_XON_XOFF
                    RTS_CTS = PER_RTS_CTS
                ...
                # EZSP protocol initialization...
                if ezspV8Init(ser)==NO_ERROR or ezspV9Init(ser)==NO_ERROR or ezspV7Init(ser)==NO_ERROR or ezspV6Init(ser)==NO_ERROR or ezspV5Init(ser)==NO_ERROR or ezspV4Init(ser)==NO_ERROR:
                    #flush input buffer
                    ser.flushInput()
                    
                    if isV8 == True or isV9 == True:
